...
def _process_image(directory, name):
    ...
    for obj in root.findall('object'):
        ...
        if bbox is not None:
            bboxes.append((max(float(bbox.find('xmin').text) / shape[0], 0.0),
                           max(float(bbox.find('ymin').text) / shape[1], 0.0),
                           min(float(bbox.find('xmax').text) / shape[0], 1.0),
                           min(float(bbox.find('ymax').text) / shape[1], 1.0)
                           ))
